...
public class MySQLProvider extends SQLProviderAdapter<MySQLGlobalState, MySQLOptions> {
    public List<Query<?>> createQueryList;
    public static String content;
    public static BufferedWriter bwUnConsistent;
    ...
    public void generateDatabase(MySQLGlobalState globalState) throws Exception {
        ...
        queries = se.queryList;

        //获得数据库表信息
        content = DatabaseInfoBuilder.InfoBuilder();
        File filewriteCom = new File("compare_result_"+globalState.getDatabaseName()+".txt");
        if(!filewriteCom.exists()){
            filewriteCom.createNewFile();
        }
        FileWriter fileWriterCom = new FileWriter(filewriteCom.getAbsoluteFile());
        bwUnConsistent = new BufferedWriter(fileWriterCom);
    }
