...
//    }
    enum ExpressionType {
        RANDOM_QUERY,COLUMN_NAME, LITERAL_VALUE, UNARY_OPERATOR, POSTFIX_UNARY_OPERATOR, BINARY_OPERATOR,
        BETWEEN_OPERATOR, BINARY_COMPARISON_OPERATOR, FUNCTION, IN_OPERATOR,
        MATCH, AGGREGATE_FUNCTION, AND_OR_CHAIN
    }
...
////        }
//        if (Randomly.getBoolean()) {
//            return new SQLite3Exists(generateSubQuery(), SQLite3Constant.createFalse());
//        } else {
//            return new SQLite3Exists("SELECT NULL", SQLite3Constant.createFalse());
//        }
//
//    }
...
//                    new SQLite3RowValueExpression(right), BinaryComparisonOperator.getRandomRowValueOperator(),null);
//        case BETWEEN:
...
//        }
    ...
    private SQLite3Expression getBinaryComparisonOperator(int depth) {
        ...
        SQLite3Expression rightExpression = getRandomExpression(depth + 1);
        return new SQLite3Expression.BinaryComparisonOperation(leftExpression, rightExpression, operator,generateSubQuery());
    }

    SQLite3Schema s;
    SQLite3Schema.SQLite3Tables targetTables;
    SQLite3ExpressionGenerator gen;
    SQLite3Select select;
    private String generateSubQuery() {
        s = globalState.getSchema();
        targetTables = s.getRandomTableNonEmptyTables();
        gen = new SQLite3ExpressionGenerator(globalState).setColumns(targetTables.getColumns());
        ...
        SQLite3Expression expression = generateExpression();
        select.setWhereClause(expression);
        //System.out.println("select * from("+ SQLite3Visitor.asString(select)+" limit 1 ) tempTable ");

        return "select * from("+ SQLite3Visitor.asString(select)+" limit 1 ) tempTable ";
    }
    ...
    @Override
    public SQLite3Expression generatePredicate(String currentCov) {
        return generateExpression();
    }
