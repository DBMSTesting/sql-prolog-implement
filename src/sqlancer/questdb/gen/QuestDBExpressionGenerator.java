...
public class QuestDBExpressionGenerator extends UntypedExpressionGenerator<Node<QuestDBExpression>, QuestDBColumn> {
    ...
    @Override
    protected Node<QuestDBExpression> generateExpression(int depth,String currentCov) {
        ...
        case BINARY_COMPARISON:
            return new NewBinaryOperatorNode<>(generateExpression(depth + 1,""), generateExpression(depth + 1,""),
                    QuestDBBinaryComparisonOperator.getRandom(),null);
        case BINARY_ARITHMETIC:
            return new NewBinaryOperatorNode<>(generateExpression(depth + 1,""), generateExpression(depth + 1,""),
                    QuestDBBinaryArithmeticOperator.getRandom(),null);
        case BINARY_LOGICAL:
            return new NewBinaryOperatorNode<>(generateExpression(depth + 1,""), generateExpression(depth + 1,""),
                    QuestDBBinaryLogicalOperator.getRandom(),null);
        case IN:
