...
public class    DuckDBJoin implements Node<DuckDBExpression> {
    ...
    public enum JoinType {
        INNER, NATURAL, LEFT, RIGHT, POSITIONAL;
    ...
    public static List<Node<DuckDBExpression>> getJoins(
        ...
        while (tableList.size() >= 2 && Randomly.getBooleanWithRatherLowProbability()) {
            ...
            case POSITIONAL:
                joinExpressions
                        .add(DuckDBJoin.createPositionalJoin(leftTable, rightTable, OuterType.getRandom()));
                break;
            default:
    ...
    public static Node<DuckDBExpression> createPositionalJoin(TableReferenceNode<DuckDBExpression, DuckDBTable> left,
                                                           TableReferenceNode<DuckDBExpression, DuckDBTable> right, OuterType naturalJoinType) {
        DuckDBJoin join = new DuckDBJoin(left, right, JoinType.POSITIONAL, null);
        join.setOuterType(naturalJoinType);
        return join;
    }
}
